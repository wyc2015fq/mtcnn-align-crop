...
class FaceAlignmentor:
    ...
    _tstart_stack = []
    def tic(self):
        self._tstart_stack.append(self.time())
    def toc(self,fmt="Elapsed: %s s"):
        print fmt % (self.time()-self._tstart_stack.pop())
    ...
    def haveFace(self, img, facedetector):
        ...
        #tic()
        boundingboxes, points = self.__detect_face(img_matlab, minsize, PNet, RNet, ONet, threshold, False, factor)
        #toc()
        containFace = (True, False)[boundingboxes.shape[0]==0]
        return containFace, boundingboxes

    def detectFaceAll(self, imgfilelist, minsize = 20,threshold = [0.6, 0.7, 0.7],factor = 0.709):
        #error = []
        ...
        for imgpath in f.readlines():
            ...
            #tic()
            boundingboxes, points = self.__detect_face(img_matlab, minsize, self.PNet, self.RNet, self.ONet, threshold, False, factor)
            #toc()
    ...
    def alignment(self,image,points,ref_points=np.array([30.2946, 51.6963, 65.5318, 51.5014, 48.0252, 71.7366, 33.5493, 92.3655, 62.7299, 92.2041]),ref_shape = (112, 96, 3)):
        ...
        #i = warpimage*255
        #cv2.imwrite('d:/1.jpg',i[0:rows,0:cols,:])
        #cv2.imshow('',warpimage[0:rows,0:cols,:])
        #cv2.waitKey(0)
        return warpimage[0:rows,0:cols,:] #warpiamge  for whole image
	
	#单张图片开始计算,Ensure设置自动降低阈值寻找人脸
...
if __name__ == "__main__":
    caffe_model_path = 'C:\Users\User\IdeaProjects\caffe_face\caffe_model_params'
    a = FaceAlignmentor(caffe_model_path)
    from PIL import Image as im
    import numpy as np
    img = im.open('C:\Users\User\IdeaProjects\caffe_face\\Hosni_Mubarak_0006.jpg')
    get_image = np.array(img)
    get_image = get_image[:, :, ::-1]
    _,p=a.detectFace(get_image)
    print p
    ...
    for j in range(5):
        points[2*j] = p[0][j]
        points[2*j+1] = p[0][j+5]
    imaged.point((points[0],points[1]))
    imaged.point((points[2],points[3]))
    imaged.point((points[4],points[5]))
    imaged.point((points[6],points[7]))
    imaged.point((points[8],points[9]))
    img.show()
